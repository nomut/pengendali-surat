...
<template>
    ...
    >
        <main>
            ...
            >
                <div class="absolute inset-0 bg-linear-to-b from-slate-900/60 via-slate-900/25 to-slate-900/50"></div>
            ...
            <section id="lebih-lanjut" class="py-20 sm:py-24 bg-white/50">
                <div class="container mx-auto px-4 sm:px-6">
                    <div v-animate-on-scroll class="max-w-2xl mx-auto text-center mb-16 animate-on-scroll">
                        <span class="inline-block mb-3 text-sm font-semibold uppercase tracking-[0.2em] text-sky-600">Program Kerja</span>
                        <h2 class="text-3xl md:text-4xl font-bold tracking-tight text-slate-900">Kelompok Kerja</h2>
                        <div class="mx-auto mt-5 h-1 w-16 rounded-full bg-linear-to-r from-sky-500 to-cyan-400"></div>
                        <p class="mt-6 text-lg leading-relaxed text-slate-600">Untuk mendukung Visi dan Misi PKK DIY, beberapa kelompok kerja yang ada.</p>
                    </div>
            ...
            <section id="kegiatan-terbaru" class="py-20 sm:py-24 bg-transparent border-t border-slate-200/50">
                <div class="container mx-auto px-4 sm:px-6">
                    <div v-animate-on-scroll class="mb-16 animate-on-scroll flex flex-col sm:flex-row sm:justify-between sm:items-end gap-6">
                        <div class="text-left">
                            <span class="inline-block mb-3 text-sm font-semibold uppercase tracking-[0.2em] text-sky-600">Berita Terkini</span>
                            <h2 class="text-3xl md:text-4xl font-bold tracking-tight text-slate-900">Kegiatan Terbaru</h2>
                            <div class="mt-5 h-1 w-16 rounded-full bg-linear-to-r from-sky-500 to-cyan-400"></div>
                            <p class="mt-6 text-lg leading-relaxed text-slate-600">Berita dan dokumentasi kegiatan terkini.</p>
                        </div>
                        <InertiaLink :href="route('kegiatan')" class="group shrink-0 inline-flex items-center gap-2 px-6 py-3 rounded-full bg-sky-600 text-white font-semibold shadow-lg shadow-sky-600/25 hover:bg-sky-700 hover:shadow-sky-600/40 transition-all duration-300">
                            Lihat Semua <ArrowRight class="w-4 h-4 transition-transform duration-300 group-hover:translate-x-1" />
                        </InertiaLink>
                    </div>
                    ...
                    <div v-if="latestActivities && latestActivities.length > 0" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                        <article 
                            ...
                            :key="activity.id"
                            v-animate-on-scroll
                            class="animate-on-scroll bg-white rounded-2xl shadow-sm hover:shadow-xl hover:shadow-sky-900/10 transition-all duration-300 overflow-hidden group flex flex-col h-full border border-slate-200/70 hover:-translate-y-1.5"
                            :style="{ 'transition-delay': `${index * 100}ms` }"
                        >
                            <InertiaLink :href="route('kegiatan.show', activity.slug)" class="block relative aspect-[16/9] overflow-hidden bg-slate-100">
                                <img 
                                ...
                                <div v-if="activity.date" class="absolute top-4 left-4 bg-white/95 backdrop-blur-sm px-3 py-1.5 rounded-lg shadow-sm font-semibold text-xs text-sky-700">
            ...
            <section id="galeri-terbaru" class="py-20 sm:py-24 bg-transparent border-t border-slate-200/50">
                <div class="container mx-auto px-4 sm:px-6">
                    <div v-animate-on-scroll class="mb-16 animate-on-scroll flex flex-col sm:flex-row sm:justify-between sm:items-end gap-6">
                        <div class="text-left">
                            <span class="inline-block mb-3 text-sm font-semibold uppercase tracking-[0.2em] text-sky-600">Dokumentasi</span>
                            <h2 class="text-3xl md:text-4xl font-bold tracking-tight text-slate-900">Galeri Kegiatan</h2>
                            <div class="mt-5 h-1 w-16 rounded-full bg-linear-to-r from-sky-500 to-cyan-400"></div>
                            <p class="mt-6 text-lg leading-relaxed text-slate-600">Arsip foto dari berbagai kegiatan.</p>
                        </div>
                        <InertiaLink :href="route('galeri')" class="group shrink-0 inline-flex items-center gap-2 px-6 py-3 rounded-full border border-sky-600 text-sky-700 font-semibold hover:bg-sky-600 hover:text-white transition-all duration-300">
                            Selengkapnya <ArrowRight class="w-4 h-4 transition-transform duration-300 group-hover:translate-x-1" />
                        </InertiaLink>
                    </div>
                    ...
                    <div v-if="latestGalleries && latestGalleries.length > 0" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                        <div 
                            ...
                            :key="gallery.id"
                            v-animate-on-scroll
                            class="animate-on-scroll group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:shadow-sky-900/10 transition-all duration-300 border border-slate-200/70 hover:-translate-y-1.5 flex flex-col h-full"
                            :style="{ 'transition-delay': `${index * 100}ms` }"
                        >
                            ...
                            <div class="p-6 flex flex-col flex-grow">
                                ...
                                <InertiaLink :href="route('galeri.show', gallery.slug)" class="block group-hover:text-sky-600 transition-colors">
                                    <h3 class="text-lg font-bold text-slate-800 mb-2 line-clamp-2 leading-tight">{{ gallery.title }}</h3>
                                </InertiaLink>
                            </div>
                        </div>
                    </div>
